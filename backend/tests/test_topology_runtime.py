...
def test_unknown_destination_floods():
    ...
    events, _, _, _ = simulate_datalink_topology(req=req, session_state=state)
    flood = [
        e for e in events
        if e.layer == "engine"
...
def test_learning_then_unicast_across_runs():
    ...
    _, _, tables1, _ = simulate_datalink_topology(req=req1, session_state=state)
    ...
    events2, _, _, _ = simulate_datalink_topology(req=req2, session_state=state)
    unicast = [
        e for e in events2
        if e.layer == "engine"
        and e.src_device == "sw1"
...
def test_destination_change_floods_then_learns_new_mac():
    ...
    events2, _, tables2, _ = simulate_datalink_topology(req=req2, session_state=state)
    flood = [
        e for e in events2
        if e.layer == "engine"
        and e.src_device == "sw1"
...
def test_reset_and_topology_change_invalidate_tables():
    ...
    _, _, tables_reset, _ = simulate_datalink_topology(req=req_reset, session_state=state)
    assert tables_reset.get("sw1", []) == []
    ...
    _, _, tables_changed, _ = simulate_datalink_topology(req=req_changed, session_state=state)
...
def test_two_switch_chain_flood_then_unicast_across_both_switches():
    ...
    req1 = _req(topology_devices=devices, topology_links=links, src_device_id="h1", dst_device_id="h2")
    events1, _, tables1, _ = simulate_datalink_topology(req=req1, session_state=state)
